'''
Turbine Bypass Valve Model v4 - February 29, 2016
Justin Gomes
Gemma Power Systems, LLC

UPDATES: 
	- Removed input functions so that the function should now be called by TBV_builder.py which has the
	built-in input fields
	- Added P12 and P22 (back pressures for condenser sparger/downstream fixtures for reference and unknown
	cases, respectively) to allow for modeling of HP bypass since it doesn't dump into the condenser which
	can be assumed to have 0 bac pressure.
...
def float_range(start, end, interval):
	'''
	float_range(start, end, interval)
	
	Creates list of floats in "interval" increments
	
	INPUT: 
		start: FLOAT, first number of the list
		end: FLOAT, last number to be in the list
	...
	list = [start]
	for n in l[1:]:
		list.append(round(interval + list[n-1], 7))
	return list
...
class back_P(object):
	...
	def __init__(self, back_input):
		...
		self.P11 = back_input[1]*6894.76 # upstream pressure for reference conditions in Pa
		...
		self.P12 = back_input[5]*6894.76 # downstream pressure for reference case (psia)
		self.P22 = back_input[6]*6894.76 # downstream pressure for unknown case (psia)
...
#			raise ValueError, "transient flow - friction coefficient cannot be calculated"
			...
			while abs(fd-fd_guess) > fd_guess/100.:
				...
				fd = (1/(-2*log10(k/(3.7*D)+2.51/(Re*sqrt(fd_guess)))))**2
	...
	def solve_back_P(self, P_high, P_low, T21, length):
		...
		S2 = steam_pT(P_guess, T21) # create instance for guessed steam conditions
	...
	def calc_back_P(self, P11 = None):
		...
		S = steam_pT(P11, self.T11) # create instance for reference steam conditions
		...
		if self.m_dot2 < self.m_dot1:
			...
			P_high = 10000*6894.76
			P_low = P11
			P_guess = self.solve_back_P(P_high, P_low, T21, length)

		return (P_guess+self.P22)/6894.76 # return guess that gives matching P_drop in psia
...
class TBV_model(back_P):
	...
	def __init__(self, TBV_input, back_input):
		...
		self.P1 = TBV_input[0]
		self.P1_SI = self.P1*6894.76
		self.T1 = TBV_input[1]
		self.T1_SI = (self.T1+491.67)*0.5555556
	...
	def calc_Cv(self, P1, P2, m_dot): # calculate Cv for given pressure drop and flow rate.  raises exception if backpressure is entered incorrectly
		...
		stm = steam_pT(P1*6894.76, self.T1_SI)
#		Fk = 1.33/1.4 # vendor may provide different value that may result in results more consistent with the vendor's - 1.33 is a mid range value of cp/cv for superheated steam
		...
		try:
			...
			raise ValueError, "Backpressure exceeds upstream pressure."
	...
	def calc_m_dot_total(self,P1 = None, m_dot = None):
		...
		P1 = P1*6894.76 # convert P1 to metric
		stm1 = steam_pT(P1, self.T1_SI)
		stm_s = steam_Tx((self.T_s+491.67)*0.5555556,0)
	...
	def calc_flow_rate(self, p, m_dot_high = 1000000.):
		...
		while abs(P_test-P2) > p/100.: # converge back pressure calcs to within 1% of upstream pressure
			...
			elif n > 100 and abs(P_test-P2) >= p/10.: raise ValueError, "TBV outlet pressure will not converge"
			if P_test < P2: 
